...
const OurGenresMovies: React.FC<Props> = ({ movies }) => {
    const [activeIndex, setActiveIndex] = useState(0);
    const swiperRef = useRef<SwiperCore | null>(null);
    const { genres } = useContext(DataContext);
    const [interval, setInterval] = useState<number>(5);
    const [mobileDesign, setMobileDesign] = useState<boolean>(false);
    const [reversedShows, setReversedShows] = useState<Movie[]>([]);
    ...
    useEffect(() => {
        setReversedShows([...movies].reverse());
    }, [movies]);
            ...
            <section className={styles['our-genres']}>
                <div className="heading">
                    ...
                    <div className="controller-pagination">
                        <button className={`g-prev-slide btn-dark pBtn`}>
                        ...
                        <button className={`g-next-slide btn-dark pBtn`}>
                            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            {Array.from({ length: 4 }).map((_, slideIndex) => (
                                <SwiperSlide key={slideIndex}>
                                    <div className={styles.slide}>
                                        {genres.slice(slideIndex * interval, (slideIndex + 1) * interval).map((item, index) => (
...
export default OurGenresMovies;
