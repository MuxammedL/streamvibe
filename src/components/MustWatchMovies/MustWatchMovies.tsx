...
const MustWatchMovies = () => {
    ...
    useEffect(() => {
        ...
        const fetchMovies = async () => {
            try {
                const response = await fetch(movieAPI({ page: 1, query: 'movie/top_rated' }));
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                const data = await response.json();
                const { results } = data;
                const sortedMovies = results.sort((a: Movie, b: Movie) => b.vote_count - a.vote_count);
        ...
        <section className='trending-now'>
            <div className="heading">
                ...
                <div className="controller-pagination">
                    <button className={`m-prev-slide btn-dark pBtn`}>
                    ...
                    <button className={`m-next-slide btn-dark pBtn`}>
                        <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
